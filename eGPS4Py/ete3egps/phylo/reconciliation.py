...
def get_reconciled_tree(node, sptree, events) -> (Tree,List[EvolEvent]):
    """
    Returns the reconciliation gene tree with a provided species topology
    The events has the following attributes.
    """
    if len(node.children) == 2:
        # First visit childs
        # 对当前内部节点，先 后序递归调用自身，对左右子树分别进行对齐处理，获取“morphed_childs”。
        morphed_childs = []
        ...
        # get_species() 方法应返回叶节点所在的物种集合。
        # all_species 是合并左右子树后所有物种的集合。
        ...
        # Check that both are reconciled to the same species
        if len(sp_child_0 & sp_child_1) > 0:
            # 若两个子树对应物种集有交集，则判定为 复制事件（D）
            # 步骤解析：
            # 复制当前节点node
            # 基于物种树 sptree 和 all_species 构造一个合理拓扑 template
            # 使用 _replace_on_template(...) 将左右子当前已 align 子树“插”进该拓扑
            # 构造新内部节点 newnode，并设置 evoltype 为 "D"
            # 创建一个 EvolEvent，记录 in-/out- paralogs 与序列信息
            # 把事件加入列表，返回拼好的树 + events
            ...
            # inparalogs/outparalogs、in_seqs/out_seqs 信息记录
            e = EvolEvent()
            e.etype = "D"
        ...
        # Otherwise, we need to reconciliation species at both sides
