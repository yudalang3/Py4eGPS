...
"""
This module defines the PhyloNode datatype to manage phylogenetic
trees. It inherits the core-type TreeNode and add some special
features to the node instances.

The PhyloTree class is an extension of the base Tree object, providing an appropriate way to deal with phylogenetic trees.
Thus, while leaves are considered to represent species (or sequences from a given species genome),
internal nodes are considered ancestral nodes.
...
class PhyloNode(TreeNode):
    ...
    def reconcile(self, species_tree):
        """
        Returns the reconciled topology with the provided species tree,
        and a list of evolutionary events inferred from such reconciliation.
        """
        return get_reconciled_tree(self, species_tree, [])

    def get_my_evol_events(self, sos_thr=0.0) -> List:
        ...
        """
        from . import spoverlap, EvolEvent
        return spoverlap.get_evol_events_from_leaf(self, sos_thr=sos_thr)

    def get_descendant_evol_events(self, sos_thr=0.0) -> List:
    ...
    def __get_speciation_trees_recursive(self, autodetect_duplications=True):
