...
class ClusterNode(TreeNode):
    ...
    of ClusterNode instances connected in a hierarchical way (Just a tree), and
    representing a clustering result.

    The basic difference between Tree and ClusterTree is that leaf nodes in a cluster-tree are linked to numerical profiles.
    Such profiles are expected to represent the data used to generate the clustering tree. In other words, trees are bound to numerical arrays.
