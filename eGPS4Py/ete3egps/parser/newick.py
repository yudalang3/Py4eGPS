...
# 在 ETE3 里用 __all__ 主要是 明确告诉用户哪些函数是“公共接口”，哪些是内部用的，避免混乱。
__all__ = ["read_newick", "write_newick", "print_supported_formats","NewickError","NW_FORMAT"]
...
def _get_features_string(self, features=None):
    ...
    a node.
    生成带扩展信息的 **NHX 格式 Newick 字符串**，包含节点的额外属性。

    NHX（New Hampshire eXtended）是 Newick 树格式的一种扩展，可以在节点后附加信息：
    例如：A[&&NHX:taxid=9606:name=Homo_sapiens]

    针对不同的feature，会尝试用str函数得到字符串
    """

    string = ""  # 最终拼接的 NHX 特征字符串

    # 1️⃣ 处理 features 参数
    if features is None:
        # 如果没传 features，默认空列表（不会输出任何扩展信息）
        features = []
    elif features == []:
        # 如果传的是空列表 []，表示“自动使用节点已有的所有 features”
        # self.features 可能是节点绑定的特征集合
    ...
    for pr in features:
        # 确认当前节点 self 是否有这个属性
        if hasattr(self, pr):
            raw = getattr(self, pr)  # 取到原始属性值

            # 根据类型不同做不同序列化处理：
            if type(raw) in ITERABLE_TYPES:
                # 如果是 list / tuple 等可迭代，就用 '|' 拼接成字符串
                raw = '|'.join(map(str, raw))

            elif type(raw) == dict:
                # 如果是 dict，拼接成 key-value 形式：key1-val1|key2-val2
                raw = '|'.join("%s-%s" % (k, v) for k, v in raw.items())
                ...
                # 其他类型（int/float/自定义对象）转成 str
                raw = str(raw)

            # 3️⃣ 过滤非法 Newick 字符（比如空格、特殊符号等）
            # 将非法字符替换成 "_"
            value = re.sub("["+_ILEGAL_NEWICK_CHARS+"]", "_", raw)

            # 4️⃣ 如果已经有内容了，用 ':' 连接下一个特征
            if string != "":
                string += ":"
    ...
    # 5️⃣ 如果有内容，包装成 NHX 扩展格式 [&&NHX:key=value:key=value]
    if string != "":
        string = "[&&NHX:" + string + "]"

    # 返回完整 NHX 扩展字符串（可能是空字符串）
    return string
