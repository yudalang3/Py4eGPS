...
## 重要这是 db 数据库文件产生的数据库文件，有缓存，所以会很快！
    ...
    if not targz_file:
        ...
        if os.path.exists("taxdump.tar.gz"):
            ...
            if local_md5 != md5_check:
                do_download = True
                print('Updating taxdump.tar.gz from NCBI FTP site (via HTTP)...\nIf too slow,down by yourself: http://ftp.ncbi.nlm.nih.gov/pub/taxonomy/taxdump.tar.gz', file=sys.stderr)
