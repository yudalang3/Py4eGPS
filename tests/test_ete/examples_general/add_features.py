...
for node in t.traverse():
    if node.name and node.name in "AEIOU":
        node.add_features(vowel=True, confidence=random.random())
    else:
        node.add_features(vowel=False, confidence=random.random())

    print(f"{node.name}, {node.vowel}, {node.confidence}")
    if node.vowel:
        print("abc")
# Now we use these information to analyze the tree.
nodes_has_vowel = t.search_nodes(vowel=True)
print("This tree has", len(nodes_has_vowel), "vowel nodes")
for node in nodes_has_vowel:
    if node.vowel:
        print("node", node.name, "has vowel", node.dist)
